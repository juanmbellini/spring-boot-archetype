...
import ${package}.security.JwtTokenGenerator;
import ${package}.services.SessionService;
...
/* package */ class JwtAgent implements JwtTokenGenerator, JwtCompiler {
     ...
     */
    /* package */ JwtAgent(@Value("${symbol_dollar}{custom.security.jwt.signing-key}") String secretKey,
                           @Value("${symbol_dollar}{custom.security.jwt.duration}") Long duration,
    ...
    private static class CustomJwtHandlerAdapter extends JwtHandlerAdapter<Jws<Claims>> {
        ...
        public Jws<Claims> onClaimsJws(Jws<Claims> jws) {
            ...
            if (userIdObject == null) {
                throw new MissingClaimException(header, claims, "Missing ${symbol_escape}"user id${symbol_escape}" claim");
            }
            // Check user id is an integer (or long) number
            if (!(userIdObject instanceof Integer) && !(userIdObject instanceof Long)) {
                throw new MalformedJwtException("The ${symbol_escape}"user id${symbol_escape}" claim must be an integer or a long");
            }
            ...
            if (jtiObject == null) {
                throw new MissingClaimException(header, claims, "Missing ${symbol_escape}"jwt id${symbol_escape}" claim");
            }
            // Check user id is an integer (or long) number
            if (!(jtiObject instanceof Integer) && !(jtiObject instanceof Long)) {
                throw new MalformedJwtException("The ${symbol_escape}"jwt id${symbol_escape}" claim must be an integer or a long");
            }
            // Transform the user id from integer to long
            ...
            if (rolesObject == null) {
                throw new MissingClaimException(header, claims, "Missing ${symbol_escape}"roles${symbol_escape}" claim");
            }
            // Check roles is a Collection
            if (!(rolesObject instanceof Collection)) {
                throw new MalformedJwtException("The ${symbol_escape}"roles${symbol_escape}" claim must be a collection");
            }
            ...
            final Date issuedAt = Optional.ofNullable(claims.getIssuedAt())
                    .orElseThrow(() ->
                            new MissingClaimException(header, claims, "Missing ${symbol_escape}"issued at${symbol_escape}" date"));
            if (issuedAt.after(new Date())) {
                throw new MalformedJwtException("The ${symbol_escape}"issued at${symbol_escape}" date is a future date");
            }
            // Check expiration date is not missing
            if (claims.getExpiration() == null) {
                throw new MissingClaimException(header, claims, "Missing ${symbol_escape}"expiration${symbol_escape}" date");
            }
